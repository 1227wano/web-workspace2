			...
			<table
				...
				<tr>
					<td><input type="text" name="userName" id="user_name"
						...
						required="required" aria-required="true"
						style="margin-bottom: 25px; width: 100%; height: 40px; border: 1px solid #d9d9de"
						placeholder="한글로 최대 6자" value="${ loginUser.userName }"></td>
				</tr>
				...
				<tr>
					<td><input type="email" name="email" id="user_email"
						...
						required="required" aria-required="true"
						style="margin-bottom: 25px; width: 100%; height: 40px; border: 1px solid #d9d9de"
						placeholder="ex) kh@kh.com" value="${ loginUser.email }"></td>
				</tr>
				...
				<script>
					...
					// 회원가입시 선택한 취미(체크박스)를 내정보에서 보이게!
					// -> HTML요소를 조작하는 것 -> 자바스크립트!
					const interest = '${ loginUser.interest }';  // 우선 interest를 자바스크립트 형식으로 바꾸기
					...
					$('input[type=checkbox]').each((e, c) =>{
						...
						// 바닐라 자바스크립트의 indexOf => 없으면 -1반환 == 문자열은 search
